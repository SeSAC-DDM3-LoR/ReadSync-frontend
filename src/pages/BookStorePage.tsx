    ...
    if (error) {
        return (
            <div className="flex flex-col items-center justify-center p-20 text-center">
                <div className="text-forest-400 mb-4 text-lg">도서 정보를 불러오는데 실패했습니다.</div>
        ...
        <div className="space-y-8">
            ...
            <div className="flex items-center gap-2 overflow-x-auto pb-2">
                ...
                {categories?.map(cat => (
                    <Button
                        key={cat.categoryId}
                        variant={selectedCategory === cat.categoryId ? 'primary' : 'outline'}
                        size="sm"
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-6">
                {isLoading ? (
                    ...
                    <Card
                        key={book.bookId}
                        className="group cursor-pointer hover:-translate-y-1 hover:shadow-xl transition-all duration-300 border-none bg-white p-0 overflow-hidden"
                    ...
                    >
                        ...
                        {/* Info */}
                        <div className="p-4">
                            <div className="text-xs text-forest-500 font-medium mb-1">{categories?.find(c => c.categoryId === book.categoryId)?.categoryName || '기타'}</div>
